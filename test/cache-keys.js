...
describe('cache-delete', () => {
  ...
  it('Cache.keys with URL', async () => {
    let cache = await caches.open('v1')
    await cache.put(new Request('http://example.com/foo'), new Response)
    let keys = await cache.keys('http://example.com/foo')
  ...
  it('Cache.keys with ignoreSearch option (request with no search parameters)', async () => {
    ...
    await cache.put(new Request('http://example.com/foo'), new Response)
    await cache.put(new Request('http://example.com/foo?foo=bar'), new Response)

    let keys = await cache.keys('http://example.com/foo', {ignoreSearch: true})
  ...
  it('Cache.keys with ignoreSearch option (request with search parameters)', async () => {
    ...
    let keys = await cache.keys('http://example.com/foo?bar=baz', {ignoreSearch: true})
