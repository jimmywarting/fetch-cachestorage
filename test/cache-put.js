...
var test_body = (body = 'Hello world!') => {
  var rs = new Readable
  rs.push(body); rs.push(null)
  return rs
}
...
describe('cache-put', () => {
  ...
  it.skip('Cache.put with an empty response body', async () => {
    let cache = await caches.open('v1')
    let request = new Request(test_url)
    let response = new Response(test_body('hej'), {
        status: 200,
  ...
  it('Cache.put called twice with matching Requests and different Responses', async () => {
    ...
    await cache.put(new Request(test_url), new Response(test_body('Old body'), { statusText: 'Old status' }))
